...
**Board groups**: `fishball_mini_7010`/`fishball_mini_7020` are flash-only builds (QSPI `.frm`/`.dfu`, no SD image — they use `postimage-qspi.sh` only) for the same physical hardware as `fishball`/`fishball7020`. Rather than shipping as their own release artifacts, each one's `boards.json` entry carries `"artifact"` pointing at its full-size counterpart (`fishball_mini_7010` → `"artifact": "fishball"`, `fishball_mini_7020` → `"artifact": "fishball7020"`) — the group name is the primary board's own name, not a synthetic label. `./build.sh fishball` builds *both* `fishball_maiasdr_defconfig` and `fishball_mini_defconfig` and merges them into one `build/fishball.zip`: since each build only ever populates one of `images/flash/` or `images/sdimg/` (the full build fills `sdimg/`, the mini build fills `flash/` — never both), the merge is a plain union with nothing to collide, no per-variant subfolder needed. This means `./build.sh fishball` now does more work than building just that one defconfig — it's a deliberate behavior change, since the merged zip is the intended release shape, not a one-off. `./build.sh fishball_mini_7010` alone still builds just that variant, unmerged, standalone. CI mirrors this: each board is its own matrix row/artifact upload (grouping only ever happens via `boards.json`'s `artifact` field, defaulting to the board's own name), and the release job's "Merge grouped board artifacts" step folds a group together — only when every member was actually built in that run (a `workflow_dispatch` for just the mini board ships its zip standalone instead).
