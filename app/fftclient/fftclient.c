                              ...
                              void *user_data)
{
	...
	*/
	...
	if (is_bin) {
		...
		{
			//fprintf(stderr,"%f - ",fdata[i]);

			float logval=10*log(fdata[i]/(4096))/3;
			if(logval>min+dbfloar)
			{
				if(inaslot==0)
				{
					Peak++;
		...
		if(oldPeak!=Peak)
		{
			fprintf(stderr,"New station\n");
			oldPeak=Peak;
		}	
		//fprintf(stderr,"min %f max %f db %f peak %d\n",min,max,max-min,Peak);
