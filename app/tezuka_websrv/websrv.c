...
//#define DOCUMENT_ROOT "/mnt/nfs/tezuka_fw/app/sweep/www"
#define DOCUMENT_ROOT "/mnt/nfs/tezuka_fw/board/pluto/msd"
...
int
MaiaHandler(struct mg_connection *conn, void *cbdata)
{
...
#define FORWARD_URL "127.0.0.1"

 	
    struct mg_connection *forward_conn;
    const struct mg_response_header *header;
    ...
    forward_conn = mg_connect_client(FORWARD_URL, FORWARD_PORT,0, err_buf, sizeof(err_buf));
    if (forward_conn == NULL) {
        mg_printf(conn, "HTTP/1.1 500 Internal Server Error\r\nContent-Type: text/plain\r\n\r\nErreur interne du serveur: %s\n", err_buf);
    ...
    // Envoyer la requête GET au serveur cible
	fprintf(stderr,"length %lld\n",request_info->content_length);
    mg_printf(forward_conn,
              "%s /api HTTP/1.1\r\n"
              "Host: 127.0.0.1:8000\r\n"
              "Connection: close\r\n"
	...
	{
		mg_printf(forward_conn,"Content-Length: %lld",request_info->content_length);
		while ((nread = mg_read(conn, buf, sizeof(buf))) > 0) {
			fprintf(stderr,"%s\n",buf);
			mg_write(forward_conn, buf, nread);
		}
	}
	mg_printf(forward_conn,"\r\n");	


	int ret = mg_get_response(forward_conn, err_buf, sizeof(err_buf), 10000);
	if (ret < 0) {
		fprintf(stderr, "Download failed: %s\n", err_buf);
		mg_close_connection(forward_conn);
		return EXIT_FAILURE;
	}

	const struct mg_response_info *ri = mg_get_response_info(forward_conn);
	if (ri == NULL) {
	...
	//fprintf(stderr,"Recu %d %lld\n",ri->status_code,ri->content_length); 
    // Lire la réponse du serveur cible et l'envoyer au client
    
    while ((nread = mg_read(forward_conn, buf, sizeof(buf))) > 0) {
		fprintf(stderr,"%s\n",buf);
        mg_write(conn, buf, nread);
    }

    // Fermer la connexion au serveur cible
    mg_close_connection(forward_conn);

    return 200; // Code HTTP 200 OK
}
...
main(int argc, char *argv[])
{
	char pathroot[255];
	if (argc>1)
		strcpy(pathroot,argv[1]);
	else
		strcpy(pathroot,DOCUMENT_ROOT);
	...
	mg_set_request_handler(ctx, "/api", MaiaHandler, 0);
